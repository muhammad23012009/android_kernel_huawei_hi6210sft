...
<<<<<<< HEAD
	...
	/*
	 *Set all interrupts to nosecure, because fiq use secure
	 */
...
<<<<<<< HEAD
...
#ifndef HISI_BALONG_ARM64
#if defined (CHIP_BB_HI6210) 
    /*规避3.10内核版本上从核无法启动的问题。
      原先主核给从核发上电的软中断时，写的软中断寄存器的值为: (gic_cpu_map[cpu]<<16)|irq
      ,irq为0，即写的值为0x00FE0000，实际上是给多个从核都发送软中断，导致从核启动失败*/
    gic_cpu_map[1] = 0x2;
    gic_cpu_map[2] = 0x4;
    gic_cpu_map[3] = 0x8;
    gic_cpu_map[4] = 0x10;
	 ...
	 *Set all interrupts to nosecure, because fiq use secure
	 */
	writel_relaxed(0xffff7fff, dist_base + GIC_DIST_IGROUP);
...
<<<<<<< HEAD
	...
	for (i = 0; i < DIV_ROUND_UP(gic_irqs, 32); i++)
		gic_data[gic_nr].saved_spi_group[i] =
...
<<<<<<< HEAD
	...
	for (i = 0; i < DIV_ROUND_UP(gic_irqs, 32); i++)
		writel_relaxed(gic_data[gic_nr].saved_spi_group[i],
...
<<<<<<< HEAD
	...
	ptr = __this_cpu_ptr(gic_data[gic_nr].saved_ppi_group);
	for (i = 0; i < DIV_ROUND_UP(32, 32); i++)
...
static void __init gic_pm_init(struct gic_chip_data *gic)
{
	...
	gic->saved_ppi_group = __alloc_percpu(DIV_ROUND_UP(32, 32) * 4,
		sizeof(u32));
	BUG_ON(!gic->saved_ppi_group);
