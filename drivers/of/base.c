...
/**
 * of_property_read_u32_array_index - Find and read an planar array of 32 bit integers
 * from a property.
 *
 * @np:		    device node from which the property value is to be read.
 * @propname:	name of the property to be searched.
 * @out_value:	pointer to return value, modified only if return value is 0.
 * @sz:		    numbers of one <array element>
 * @index:      number of array elements
 * @.e.g .dts   xxx-yyy-zzz = <0xff 0xff 0xff>,
 *							<0xff 0xff 0xff>;
 *
 * The out_value is modified only if a valid u32 value can be decoded.
 */
int of_property_read_u32_planar_array(const struct device_node *np,
				       const char *propname,
				       u32 *out_value,size_t sz,u32 index)
{
