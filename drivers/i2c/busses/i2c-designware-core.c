...
static void i2c_reset_gpio(struct dw_i2c_dev *dev, int gpio_scl, int gpio_sda)
{
	int ret;
	ret = gpio_request(gpio_scl, "I2C_SCL_GPIO");
	if (ret < 0) {
		dev_err(dev->dev, "%s, gpio_request %d with"
				" ret: %d.\n", __func__, gpio_scl, ret);
		goto Done;
	}

	ret = gpio_direction_output(gpio_scl, 0);
	if (ret < 0) {
		dev_err(dev->dev, "%s, gpio_direction_output %d with"
				" ret: %d.\n", __func__, gpio_scl, ret);
		goto err_2;
	}
	udelay(5);
	...
	ret = gpio_request(gpio_sda, "I2C_SDA_GPIO");
	if (ret < 0) {
		dev_err(dev->dev, "%s, gpio_request %d with"
				" ret: %d.\n", __func__, gpio_sda, ret);
		goto err_2;
	}
	...
	if (-ETIMEDOUT == ret) {
		...
		if(++(dev->timeout_count) >= 2){
			...
			if (r<0)
				dev_warn(dev->platform_dev,
						"pins are not configured from the driver\n");
			i2c_reset_gpio(dev, dev->gpio_scl, dev->gpio_sda);
			r = devm_pinctrl_state_select(dev, PINCTRL_STATE_DEFAULT);
			if (r<0)
				dev_warn(dev->platform_dev,
						"pins are not configured from the driver\n");
