...
{
	struct fpsimd_state *fpsimd_hot_regs = NULL;

	fpsimd_hot_regs = &per_cpu(fpsimd_hot_state, (unsigned int)hcpu);

	switch (action) {
#ifdef CONFIG_HOTPLUG_CPU
	case CPU_DYING:
	case CPU_DYING_FROZEN:
		fpsimd_save_state(fpsimd_hot_regs);
		break;

	case CPU_STARTING:
	case CPU_STARTING_FROZEN:
		fpsimd_load_state(fpsimd_hot_regs);
		break;
#endif
...
static struct notifier_block fpsimd_cpu_hot_notifier_block = {
