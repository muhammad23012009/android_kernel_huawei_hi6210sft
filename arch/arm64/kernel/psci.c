...
#if defined(CONFIG_HI3XXX_SR) || defined(CONFIG_HI6XXX_SR)
	struct psci_power_state sr_state;

	if (index == 3) {
		sr_state.id = 0;
		sr_state.type = 1;
		sr_state.affinity_level = 1;

		return psci_ops.cpu_suspend(sr_state, virt_to_phys(cpu_resume));
	}
#endif
