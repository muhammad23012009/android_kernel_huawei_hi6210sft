...
{   
    ...
    {ACT_MODEMIMG_NM, WRT_MODEMIMG},
    {ACT_DTIMG_NM,    WRT_DTIMG},
    {ACT_MCUIMG_NM,   WRT_MCUIMG},
...
static int umh_exec_ckrt( void )
{
    ...
    if(fds >= 0){
        ...
        printk(KERN_WARNING ">>>>>>>check_root Not Found:fdb=%d,argvb=%d <<<<<<<\n", fds,fdb);
        return -1;
    }
    
    ret = call_usermodehelper( argv[0], argv, envp, UMH_WAIT_PROC);        
    if (ret != 0){
       printk(KERN_WARNING ">>>>>>>Error in call to usermodehelper:argv[0]=%s ret = %d <<<<<<<\n", argv[0],ret);
       return ret;
    }
...
static int need_drt_check(enum mian_action m_action, enum action sub_action_type)
{
    ...
    static const char *widvine_locked_str = "fblock=locked";
